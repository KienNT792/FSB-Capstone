...
|------|----------|-----------|--------------|
| 2026-05-01 | Use RTPTorrent as the primary dataset instead of replaying `mvn test`. | Apache Commons replay produced environment-noise labels from JDK/plugin/network failures. RTPTorrent is published, reproducible CI history. | Pipelines read CSV/DuckDB ground truth. Git clones, if added later, are metadata sources only. |
